...
// Semaphore for mutex for WordList struct access
sem_t semaphore;

/** A struct which holds all submitted words, indexed to
  * match up with the user that submitted them, along with
  * the value of each word. Length keeps track of the number
  * of words in the struct, capacity is the max number of 
  * words in the struct (for use in dynamically expanding array
  */
typedef struct {
  char **words;
  char **users;
...
// Initialize the list as a global struct
WordList *list;
...
/** Initialize the WordList struct and allocate memory */
...
/** Method for inserting a word into the WordList struct
  * when the user has not yet submitted a word.
  */
void insertNewWord(char *word, char *userName, FILE *fp) {
  sem_wait(&semaphore); // Mutex for the WordList struct
  if(list->length == list->capacity) { // Expansion if necessary
    list->capacity *= 2;
  ...
  userName[strlen(userName) - 1] = '\0'; // Cut out the newline char
  word[strlen(word) - 1] = '\0'; // Cut out the newline char

  char *allocatedUsername = (char *)malloc(sizeof(char) * strlen(userName));
  char *allocatedWord = (char *)malloc(sizeof(char) * strlen(word));

  strcpy(allocatedUsername, userName); // So that multiple entrances don't reference each other
  strcpy(allocatedWord, word); // See above comment

  list->words[list->length] = allocatedWord;
  list->users[list->length] = allocatedUsername;
  list->values[list->length++] = calculateValue(word);
  sem_post(&semaphore); // Exit mutex
}

/** Method for inserting a word into the WordList struct
  * when the user has submitted a word.
  */
void insertReplacementWord(char *word, char* userName, FILE *fp) {
  sem_wait(&semaphore); // Mutex for editing WordList
  for(int i = 0; i < list->length; i++) {
	...
	return;
      ...
      word[strlen(word) - 1] = '\0'; // Remove the newline char

      list->words[i] = word; // It's OK that the reference is the same, since the same user is submitting a word
      list->values[i] = calculateValue(word);
      sem_post(&semaphore); // Exit mutex
      return;
    }
  }
  sem_post(&semaphore); // What should be unreachable code
  fail("Bro, how the h*ck did you get here?");
}

/** Method for freeing memory in the WordList struct. However,
  * given the context of the problem, this method will not
  * be reached.
  */
void freeWordList() {
  free(list->words);
...
void *handleClient( void *args ) {
  ...
  while ( strcmp( cmd, "quit" ) != 0 ) {
	...
	fprintf(fp, "cmd> ");
      ...
      // Temp storage for running selection sort on the contents of the WordList
      char *sortedWords[n];
      int sortedValues[n];
      char *sortedNames[n];
