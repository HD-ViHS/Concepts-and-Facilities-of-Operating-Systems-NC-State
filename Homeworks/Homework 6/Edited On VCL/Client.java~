...
public class Client {
  public static void main( String[] args ) {
    ...
    try {
      ...
      byte[] rawChallengeCipher = RSAEncrypter.doFinal();
      Server.putMessage( output, rawChallengeCiper );
      
      // Get the symmetric key from the server and make AES
      // encrypt/decrypt objects for it.
      byte[] encryptedSessionKey = Server.getMessage(input);
      byte[] decryptedSessionKey = RSADecrypter.doFinal(encryptedSessionKey);
	...
	Server.putMessage( output, sessionKeyEncrypter.doFinal( request.getBytes() ) );

        // Read and print the response.
        System.out.print( new String( sessionKeyDecrypter.doFinal( Server.getMessage( input ) ) ) );
