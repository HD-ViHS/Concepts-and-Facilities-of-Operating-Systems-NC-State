...
public class Server {
  ...
  public void handleClient( Socket sock ) {
	...
	putMessage(output, ecryptedSessionKey);

        // Make AES cipher objects to encrypt and decrypt with
        // the session key.
        SecretKey key = new SecretKeySpec( sessionKey, "AES" );
	Cipher sessionKeyEncrypter = Ciper.getInstance( "AES/ECB/PKCS5Padding" );
	sessionKeyEncrypter.init( Cipher.ENCRYPT_MODE, key );

	Cipher sessionKeyDecrypter = Ciper.getInstance( "AES/ECB/PKCS5Padding" );
	sessionKeyDecrypter.init( Cipher.DECRYPT_MODE, key );
	  ...
	  if ( request.startsWith( "query " ) ) {
	    ...
	    String word = requestScanner.next();
	    ...
	    if( word.length() > WORD_MAX || requestScanner.hasNext() || ! word.matches( "[a-zA-Z]+" ) ) { 
	      ...
	      Submission test = new Submission( rec.name, word, word.getPointValue() );
	      synchronized ( lock ) { // Block access to the shared submissions state
	        if(submissions.contains(test)) {
		  Submission submissionToEdit = submissions.get(submissions.indexOf(test));
		  submissionToEdit.setValue(test.getValue());
		  submissionToEdit.setWord(test.getWord());
		} else {
	    ...
	    if(! requestScanner.next().equals("report") || requestScanner.hasNext() ) { 
	      ...
	      Comparator c = new SortByValue(); // Comparator to sort submissions by value
	      synchronized( lock ) { // Block access to the shared submissions state
