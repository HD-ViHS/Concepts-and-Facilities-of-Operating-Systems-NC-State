...
int query(AccountList *alist, char const *name )
{
  ...
  for(int i = 0; i < alist->size; i++) {
    if(strcmp(name, alist->names[i]) == 0) { // Search for account in shared memory
      int intToPrint = alist->balances[i]; // Retrieve correct amount
      #ifndef UNSAFE
        sem_post(semaphore);
      #endif
      return intToPrint; // Change to int num of cents
    }
  }
  #ifndef UNSAFE
    sem_post(semaphore);
...
int main( int argc, char *argv[] )
{
  ...
  if(strcmp(argv[1], "credit") == 0) {
    ...
    int value = query(accounts, argv[2]);
    if (value == -1) {
      fail("error");
    }
    double numToPrint = (double)value / 100;
    if (printf("%.2f\n", numToPrint) < 1) {
      fail("error");
    }
